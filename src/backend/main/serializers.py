...
class UserRegistrationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'last_name', 'user_type', 'phone', 'address', 'national_id',
...
class UserProfileSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = [
            ...
            'phone', 'address', 'national_id', 'profile_picture',
            'province', 'district', 'ds_division', 'grama_niladhari_division',
...
class ProvinceSerializer(serializers.ModelSerializer):
    name = serializers.CharField(source='name_en', read_only=True)  # Default to English name
...
class DistrictSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = District
        fields = ['id', 'name', 'name_en', 'name_si', 'name_ta', 'province', 'province_name']
...
class DSDivisionSerializer(serializers.ModelSerializer):
    ...
    province_name = serializers.CharField(source='district.province.name_en', read_only=True)
    
    class Meta:
        model = DSDivision
        fields = ['id', 'name', 'name_en', 'name_si', 'name_ta', 'district', 'district_name', 'province_name']
...
class GramaNiladhariDivisionSerializer(serializers.ModelSerializer):
    ...
    district_name = serializers.CharField(source='ds_division.district.name_en', read_only=True)
    province_name = serializers.CharField(source='ds_division.district.province.name_en', read_only=True)
    
    class Meta:
        model = GramaNiladhariDivision
        fields = [
            'id', 'name', 'name_en', 'name_si', 'name_ta', 'ds_division', 
            'ds_division_name', 'district_name', 'province_name'
        ]
...
class IssueEscalationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'from_level', 'to_level', 'reason', 'from_user_name', 
            'to_user_name', 'escalated_at'
        ]
...
class PublicCommentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = PublicComment
        fields = [
            'id', 'author_name', 'content', 'language', 'is_anonymous',
            'is_approved', 'created_at'
        ]
...
class IssueListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Issue
        fields = [
            'id', 'reference_number', 'title', 'description', 'language', 'status',
...
class EscalatedIssueSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'district_name', 'ds_division_name', 'gn_division_name',
            'current_handler_name', 'current_level', 'escalation_count',
            'response_count', 'latest_escalation', 'original_level',
    ...
    def get_latest_escalation(self, obj):
        ...
        if latest:
            return {
                ...
                'from_user': latest.from_user.get_full_name() if latest.from_user else None,
...
class IssueDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'reference_number', 'title', 'description', 'language', 'status',
            'priority', 'reporter_name', 'reporter_phone', 'reporter_address',
            'is_anonymous', 'province_name', 'district_name', 'ds_division_name',
            'gn_division_name', 'current_handler_name', 'current_level',
            'escalation_count', 'next_escalation_date', 'pending_extension_count',
...
class IssueCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Issue
        fields = [
            'title', 'description', 'category', 'language', 'priority',
            'province', 'district', 'ds_division', 'gn_division',
            'address', 'is_anonymous',
            'anonymous_name', 'anonymous_phone', 'anonymous_address', 'anonymous_id'
        ]
    ...
    def create(self, validated_data):
        ...
        if validated_data.get('is_anonymous', False):
            ...
            validated_data['reporter_address'] = validated_data.pop('anonymous_address', '')
            validated_data['reporter_national_id'] = validated_data.pop('anonymous_id', '')
        else:
            # For logged-in users, use their profile data
            if request and request.user.is_authenticated:
                validated_data['reporter_user'] = request.user
                validated_data['reporter_name'] = request.user.get_full_name() or request.user.username
                ...
                if not validated_data.get('reporter_name'):
                    validated_data['reporter_name'] = validated_data.pop('anonymous_name', 'Anonymous User')
                validated_data['reporter_phone'] = validated_data.pop('anonymous_phone', '')
                validated_data['reporter_address'] = validated_data.pop('anonymous_address', '')
                validated_data['reporter_national_id'] = validated_data.pop('anonymous_id', '')
...
class NotificationSerializer(serializers.ModelSerializer):
    class Meta:
        model = Notification
        fields = [
            'id', 'notification_type', 'title', 'message', 'issue',
            'is_read', 'created_at'
        ]
